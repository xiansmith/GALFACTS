...
void chebyshev_fit_bw_day(float *X, float *Y, int size, float nsigma, float *C, int order)
{
/*
...
/*	
	...
	for(i=2; i<size-2; i+=2) 
		{
		x[M] = X[i]; y[M] = (Y[i-1] + Y[i] + Y[i+1])/3.0; M++;
		} 
...
void chebyshev_fit_bw_scan(float *X, float *Y, int size, float nsigma, float *C, int order)
{
/*
 Basketweaving fitting function: fit + outlier removal
*/
...
*/
	...
	x[0] = X[0]; y[0] = Y[0]; M = 1; 
	for(i=5; i<size-5; i+=5) 
		{
		x[M] = X[i]; y[M] = (Y[i-4] + Y[i-3] + Y[i-2] + Y[i-1] + Y[i] + Y[i+1] + Y[i+2] + Y[i+3] + Y[i+4])/9.0; M++;
		} 
	x[M] = X[size-1]; y[M] = Y[size-1]; size = M + 1;
