...
void init_psf_map(float fwhm, float cellsize, int beamwidths)
{
	...
	psf_map = malloc (sizeof(float) * psf_map_size*psf_map_size);
	for (i=0; i<psf_map_size; i++) {
		for (j=0; j<psf_map_size; j++) {
			int xpos = i-radius;
			int ypos = j-radius;
			float dist = sqrt(xpos*xpos + ypos*ypos) * scale;
...
static float * psf_lookup_table;
static float psf_step;
static int psf_size;
	...
	for (r=0; r<daydata->numRecords; r++) 
	{
		...
		for (i=0; i<psf_map_size; i++) 
		{
			for (j=0; j<psf_map_size; j++) 
			{
				...
				if ( (m >= 0) && (m < n1) ) {
					if( (n >= 0) && (n < n2) ) {
						float spread = psf_map[j*psf_map_size+i];
						int indx = n*n1+m;
			...
			for (n=-patch; n<=patch; n++) 
			{
				...
				if ( (i >= 0) && (i < n1) ) {
					if( (j >= 0) && (j < n2) ) {
						float dx = x - (float)i;
						float dy = y - (float)j;
						float distance = dist(dx, dy, scale);
						float spread = psf(distance);
						int indx = i+j*n1;
						dataI[indx] +=  rec->stokes.I * spread;
						dataQ[indx] +=  rec->stokes.Q * spread;          
						dataU[indx] +=  rec->stokes.U * spread;          
	...
	for(j=0;j<n2;j++) 
		...
		for(i=0;i<n1;i++) 
			...
			if(w > 2.0)
				...
				dataU[i+j*n1] /= w;
				dataV[i+j*n1] /= w;
				if(dataI[i+j*n1] > 20000 || dataI[i+j*n1] < -1000) dataI[i+j*n1] = NAN;
				if(dataQ[i+j*n1] > 2000 || dataQ[i+j*n1] < -2000) dataQ[i+j*n1] = NAN;
				if(dataU[i+j*n1] > 2000 || dataU[i+j*n1] < -2000) dataU[i+j*n1] = NAN;
				if(dataV[i+j*n1] > 2000 || dataV[i+j*n1] < -2000) dataV[i+j*n1] = NAN;
				} 
			else 
				{
				dataI[i+j*n1] = NAN;
