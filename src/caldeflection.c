...
int main(int argc,char* argv[])
{
	...
	sprintf(specfilename,"%s.%s.b%1ds%1d.caldef.dat",proj_code,date,beam,band);
	...
	for(f=0;f<num_files;f++)
	{
		...
		char filepath[128];
		sprintf(filepath,"%s/%s",datadir,datafilename);
...
*/
			for(k=0;k<DUMPS_PER_ROW;k++)
			{
				...
				double Acal =0,Bcal =0,Ucal =0 ,Vcal =0;
				for(l=0;l<RAW_CHANNELS;l++)
				{
					...
					Acal = Acal + (Aon[l] - Aoff[l]);
					Bcal = Bcal + (Bon[l] - Boff[l]);
					Ucal = Ucal + (Uon[l] - Uoff[l]);
					Vcal = Vcal + (Von[l] - Voff[l]);
...
//					YXoff[l] = Voff[l];
				...
				fprintf(specfile,"%2.6f %2.6f %2.6f %2.6f\n",Acal/RAW_CHANNELS,Bcal/RAW_CHANNELS,Ucal/RAW_CHANNELS,Vcal/RAW_CHANNELS);
