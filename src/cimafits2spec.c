...
//	pdev_datum pdatum[DUMPS_PER_ROW];
	...
	for(f=0;f<num_files;f++)
	{
		...
		//		printf("%f %f %f\n",spointing_htls[h].raj_true_in_degrees,spointing_htls[h].decj_true_in_degrees,spointing_htls[h].arecibo_local_mean_sidereal_time_in_sec);
	...
	//					printf("num on %d \n",num_on);
						for(l=0;l<RAW_CHANNELS;l++)
						{
							spolset_htls_on[h2].A[l] /= num_on;
							spolset_htls_on[h2].B[l] /= num_on;
							spolset_htls_on[h2].U[l] /= num_on;
							spolset_htls_on[h2].V[l] /= num_on;
						}
						num_on = 0;
						h2++;
					}
	...
	//					printf("num off %d \n",num_off);
						for(l=0;l<RAW_CHANNELS;l++)
						{
							spolset_htls_off[h1].A[l] /= num_off;
							spolset_htls_off[h1].B[l] /= num_off;
							spolset_htls_off[h1].U[l] /= num_off;
							spolset_htls_off[h1].V[l] /= num_off;
						}
						num_off = 0;
						h1++;
					}
			...
			for(h = 0;h < DUMPS_PER_ROW/(lths_time_comp*2);h++)			
			{
				...
				for(k = 0;k < lths_time_comp*2;k++)
				{
					...
					if(num_on == lths_time_comp)
					{
						for(l=0;l<RAW_CHANNELS;l++)
						{
							spolset_lths_on[h2].A[l] /= num_on;
							spolset_lths_on[h2].B[l] /= num_on;
							spolset_lths_on[h2].U[l] /= num_on;
							spolset_lths_on[h2].V[l] /= num_on;
						}
						num_on = 0;
						h2++;
					}
					if(num_off == lths_time_comp)
					{
						for(l=0;l<RAW_CHANNELS;l++)
						{
							spolset_lths_off[h1].A[l] /= num_off;
							spolset_lths_off[h1].B[l] /= num_off;
							spolset_lths_off[h1].U[l] /= num_off;
							spolset_lths_off[h1].V[l] /= num_off;
						}
						num_off = 0;
						h1++;
					}
