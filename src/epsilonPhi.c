...
static void print_usage(char * argv[])
{
	...
	printf("Usage: %s <cube prefix> <beam> <maxpx> <maxy> <rad> <lowchan> <highchan>\n", argv[0]);
}
...
int main(int argc, char *argv[]) 
{
	...
	rad = atoi(argv[5]);
	lowchan = atoi(argv[6]);
	highchan = atoi(argv[7]);

	sprintf(cubename,"%s_BEAM%d_%04i_%04i_Icube.fits",prefix,beam,lowchan,highchan-1);
	...
	for(i=0;i<num_planes;i++)
	{
		...
		if(rad == -1)
		{
			sI[i] = plane_data[maxy*Ihpar.naxis[0]+maxx];
		}
		else
		{
		/*	count = 0;
