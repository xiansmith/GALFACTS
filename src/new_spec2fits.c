...
static void write_band_average(SpecRecord dataset[], int size, const char * fileroot)
{
	...
	fprintf(file, "#RA DEC AST CALON(A U V B) CALOFF(A U V B)\n");
	for (n=0; n<size; n++)
	{
		memset(&avgcalon, 0, sizeof(PolAvg));
		memset(&avgcaloff, 0, sizeof(PolAvg));
		count = 0;
		for (chan=0; chan<MAX_CHANNELS1; chan++) 
		{
			avgcalon.A += (unsigned long int)dataset[n].calon.A[chan];
			avgcalon.U += (long int)dataset[n].calon.U[chan];
			avgcalon.V += (long int)dataset[n].calon.V[chan];
			avgcalon.B += (unsigned long int)dataset[n].calon.B[chan];
			avgcaloff.A += (unsigned long int)dataset[n].caloff.A[chan];
			avgcaloff.U += (long int)dataset[n].caloff.U[chan];
			avgcaloff.V += (long int)dataset[n].caloff.V[chan];
			avgcaloff.B += (unsigned long int)dataset[n].caloff.B[chan];
			count++;
		}
		if (count <= 0) continue;
		fprintf(file, "%7.6f %7.6f %7.2f %lu %lu %ld %ld %lu %lu %ld %ld\n", 
			dataset[n].RA, dataset[n].DEC, dataset[n].AST,
			avgcalon.A/chan, avgcalon.B/chan, avgcalon.U/chan, avgcalon.V/chan, 
			avgcaloff.A/chan, avgcaloff.B/chan, avgcaloff.U/chan, avgcaloff.V/chan );
	}
...
static void write_time_average(SpecRecord dataset[], int size, const char * fileroot)
{
	...
	fprintf(file, "#CALON(A U V B) CALOFF(A U V B)\n");
	for (chan=0; chan<MAX_CHANNELS1; chan++)
	{
		int count = 0;
		memset(&avgcalon, 0, sizeof(PolAvg));
		...
		fprintf(file, "%lu %lu %ld %ld %lu %lu %ld %ld\n", 
			avgcalon.A/count, avgcalon.B/count, avgcalon.U/count, avgcalon.V/count, 
			avgcaloff.A/count, avgcaloff.B/count, avgcaloff.U/count, avgcaloff.V/count);
	}
...
/*static void write_noise_measurements(SpecRecord dataset[], int size, const char* fileroot, int chan)
{
	...
	unsigned int *Aon;
	unsigned int *Aoff;
	unsigned int *Adiffon;
	unsigned int *Adiffoff;
	unsigned int *Bon;
	unsigned int *Boff;
	unsigned int *Bdiffon;
	unsigned int *Bdiffoff;
...
static void process_dataset(const char * filepath, int smooth)
{	
	...
	df = cfgData.bandwitdhkHz / MAX_CHANNELS1 / 1000;
	printf("center frequency: %fMHz\n", fcen);
	printf("channel bandwidth: %fMHz\n", df);

	/* Read Datafile */
	printf("reading data file %s ... \n", datafilepath);
	numRecords = read_datafile(datafile, &dataset);
	fclose(datafile);
...
//	write_rfi_data(dataset, numRecords, filename, fcen, df);
	...
	write_pointing(dataset, numRecords, filepath);

	printf("writing band average data ...\n");
//	write_band_average(dataset, 1000, filepath);
	write_band_average(dataset, numRecords, filepath);

	printf("writing time average data ...\n");
//	write_time_average(dataset, 1000, filepath);
	write_time_average(dataset, numRecords, filepath);
...
*/
	process_dataset(filename, smooth);
	return EXIT_SUCCESS;
}
