...
static int fluxdata_read(FluxData *dataset, FILE *infile, int beam)
{
	...
	// determine the number of Records, allocate the dataser record array
    	numRecords = line_count(infile)-1;
...
{
        char fluxfilename[32+1],*datapath,*day;
        int numRecords,beam,chan;
        FluxData dataset;
        FILE *fluxfile;
        if (argc != 5) 
	{
                printf("Usage: %s <datapath> <day> <beam> <channel>\n", argv[0]);
                exit(1);
        } 
        else
        { 
	...
	sprintf(fluxfilename,"%s/%s/beam%d/fluxtime%04i.dat",datapath,day,beam,chan);
